...
import { Pencil, Power, Building2, Hotel, Coffee, UtensilsCrossed } from "lucide-react";
...
const BusinessTable = ({ businesses, onToggleStatus, onDelete }: BusinessTableProps) => {
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-border">
            <th className="text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider px-6 py-4">Business Name</th>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{typeLabels[biz.type]}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4 text-sm text-foreground">{biz.adminUsername}</td>
                <td className="px-6 py-4">
                  <span className={`text-xs px-3 py-1 rounded-full font-medium ${planColors[biz.type] || "bg-muted text-muted-foreground"}`}>
                    {typeLabels[biz.type]}
                  </span>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      size="icon"
                      className="h-8 w-8"
                      onClick={() => onToggleStatus(biz.id, biz.status)}
                    >
