...
import {
  ...
  TrendingUp, DollarSign, Send, Volume2,
} from "lucide-react";
import {
...
const statusConfig: Record<string, { label: string; icon: any; color: string; bgColor: string }> = {
  pending: { label: "Sugaya", icon: Clock, color: "text-amber-600", bgColor: "bg-amber-500/15 border-amber-500/30" },
  preparing: { label: "Diyaarinaya", icon: ChefHat, color: "text-blue-600", bgColor: "bg-blue-500/15 border-blue-500/30" },
  ready: { label: "Diyaar", icon: CheckCircle2, color: "text-green-600", bgColor: "bg-green-500/15 border-green-500/30" },
  delivered: { label: "La Geeyay", icon: Package, color: "text-muted-foreground", bgColor: "bg-muted/50 border-border" },
  cancelled: { label: "La Joojiyay", icon: XCircle, color: "text-destructive", bgColor: "bg-destructive/10 border-destructive/30" },
  paid: { label: "La Bixiyay", icon: DollarSign, color: "text-accent", bgColor: "bg-accent/15 border-accent/30" },
};
...
const WaiterDashboard = () => {
  ...
  // Real-time polling for order updates & feedback
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Check for status changes on waiter's orders
      ...
      // Check for new admin feedback messages
  ...
  const playNotificationSound = (type: "ready" | "update" | "message") => {
    try {
      ...
      const notes = type === "ready"
        ? [523.25, 659.25, 783.99, 1046.5] // ascending chime
        : type === "message"
        ? [880, 1108.73, 880] // bell
        : [659.25, 783.99]; // simple ding
  ...
  const placeOrder = () => {
    ...
    // Add waiter info for admin visibility
    (order as any).waiterId = waiter.id;
    (order as any).waiterName = waiter.name;
    (order as any).customerName = `🧑‍🍳 ${waiter.name} (Waiter)`;
  ...
  const sendMessage = () => {
    ...
    allMessages.push({
      ...
      businessId: business.id,
      from: "customer", // uses same channel as customer feedback
    ...
    toast.success("Fariin loo diray Admin-ka ✓");
    setMessageText("");
  ...
  // Notifications: admin messages + status changes
  const allMessages = JSON.parse(localStorage.getItem("dp_order_messages") || "[]");
  ...
  const tabs = [
    ...
    { id: "orders" as const, label: "Orders", icon: ClipboardList, badge: activeOrders.length },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-card/50 border-b border-border px-4 sm:px-6 py-2">
        <div className="max-w-7xl mx-auto flex items-center gap-4 sm:gap-6 text-xs overflow-x-auto">
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <span className="text-muted-foreground">Maanta:</span>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {filteredItems.length === 0 ? (
                  <div className="col-span-2 bg-card border border-border rounded-xl p-12 text-center">
                    <p className="text-muted-foreground">Wax cunto ah la ma helin</p>
                  </div>
        ...
        {activeTab === "orders" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground">{waiterOrders.length} total</span>
            </div>

            {/* Filter chips */}
            <div className="flex gap-2 flex-wrap">
              {[
                { key: "all", label: "Dhammaan", count: waiterOrders.length },
                { key: "pending", label: "⏳ Sugaya", count: waiterOrders.filter(o => o.status === "pending").length },
                { key: "preparing", label: "👨‍🍳 Diyaarinaya", count: waiterOrders.filter(o => o.status === "preparing").length },
                { key: "ready", label: "✅ Diyaar", count: waiterOrders.filter(o => o.status === "ready").length },
                { key: "delivered", label: "📦 La Geeyay", count: waiterOrders.filter(o => o.status === "delivered").length },
              ].map(f => (
                <Button
                  key={f.key}
                    ...
                    {adminMessages.length > 0 && (
                      <div className="mb-3 space-y-1.5">
                        <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                          <MessageSquare className="w-3 h-3" /> Admin Messages
                        </p>
        ...
        {activeTab === "notifications" && (
          <div className="space-y-4">
            <h2 className="text-xl font-display font-bold flex items-center gap-2">
              <Bell className="w-5 h-5 text-accent" /> Ogeysiisyada
            </h2>

            {waiterNotifications.length === 0 && activeOrders.length === 0 ? (
              <div className="bg-card border border-border rounded-xl p-12 text-center">
                <Bell className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
                <p className="text-muted-foreground">Ogeysiis la ma helin</p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                {activeOrders.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Dalabyo Active ah</p>
                    {activeOrders.map(o => {
                      const cfg = statusConfig[o.status] || statusConfig.pending;
                      const StatusIcon = cfg.icon;
                      return (
                ...
                {waiterNotifications.length > 0 && (
                  <div className="space-y-2 mt-4">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Fariimaha Admin-ka</p>
          ...
          <div className="max-w-md mx-auto space-y-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              <h3 className="font-display font-bold text-lg mb-4 flex items-center gap-2">
                <TrendingUp className="w-5 h-5 text-accent" /> Performance
              </h3>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: "Total Orders", value: waiterOrders.length, icon: "📋" },
                  { label: "Today's Orders", value: todayOrders.length, icon: "📦" },
                  { label: "Total Revenue", value: `$${waiterOrders.reduce((s, o) => s + o.total, 0).toFixed(2)}`, icon: "💰" },
                  { label: "Today's Revenue", value: `$${todayRevenue.toFixed(2)}`, icon: "📈" },
                  { label: "Delivered", value: waiterOrders.filter(o => o.status === "delivered" || o.status === "paid").length, icon: "✅" },
                  { label: "Cancelled", value: waiterOrders.filter(o => o.status === "cancelled").length, icon: "❌" },
                ].map(stat => (
      ...
      <Dialog open={!!messageDialog} onOpenChange={() => { setMessageDialog(null); setMessageText(""); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Fariin U Dir Admin-ka</DialogTitle>
            <DialogDescription>Order #{messageDialog?.orderId.slice(4, 12)}</DialogDescription>
          </DialogHeader>
          <div className="space-y-3">
            <div className="flex gap-2 flex-wrap">
              {[
                "Macmiilku wuu sugayaa ⏳",
                "Table-ka wuu buuxay 🪑",
                "Macmiilku wax cusub ayuu dalbay 📝",
                "Cuntada diyaar ma tahay? 👨‍🍳",
              ].map(q => (
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => { setMessageDialog(null); setMessageText(""); }}>Jooji</Button>
            <Button variant="hero" onClick={sendMessage} disabled={!messageText.trim()}>
              <Send className="w-4 h-4 mr-1.5" /> U Dir
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
